...
**Safety rules as invariants.** Hard finger sessions are forced ≥48h apart (72h after pain or injury history), max-hangboarding is gated on experience, and a rising acute:chronic workload ratio (>1.3) caps the coming week's intensity. Pain reports (severity 2+) substitute out finger-loading work (finger/wrist) or all high-intensity work (elbow/shoulder) for 14 days — swapped sessions say exactly what they replaced, computed by diffing against the counterfactual no-pain plan, and clearing the pain in an updated log restores the original plan. These are encoded as tests, not comments — the test suite is the contract that adaptive features can't quietly weaken a safety rule.
