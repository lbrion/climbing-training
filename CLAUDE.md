...
| History/stats shown in the History view (PRs, completion %, weekly load bars, 12-week trendlines)                                                                | `metrics.ts`                                                                                                                                                                                     |
...
| Watch/FIT file import              | `parseFit` + `POST /api/import/fit` in the same file — decodes standard FIT fields plus COROS vendor split fields (71 = send/attempt outcome, 15/16 = per-climb HR; verified against the COROS app) via `@garmin/fitsdk` with `includeUnknownData`, appends an `imported-activity` event, dedupes by `externalId`. Grades are not in the file — they exist only in COROS's app. The response `report` echoes message structure for future field mapping |
